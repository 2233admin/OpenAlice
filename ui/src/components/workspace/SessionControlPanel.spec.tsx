...
it('confirms an interruption before sending the execution ID', async () => {
  ...
  const confirmation = screen.getByRole('alertdialog', { name: 'Interrupt Scan Open?' })
  ...
  expect(interrupt).not.toHaveBeenCalled()
  fireEvent.click(screen.getByRole('button', { name: 'Interrupt run' }))
  fireEvent.click(within(screen.getByRole('alertdialog')).getByRole('button', { name: 'Interrupt run' }))
