...
export type ExecutionPhase = 'starting' | 'running' | 'stopping' | 'ended' | 'failed'
...
export interface ExecutionDriver<T> {
  ...
  start(report: (activity: string) => Promise<void>): Promise<{ value: T; pid?: number; taskId?: string; completed: Promise<{ reason: string; failed?: boolean }> }>
...
const phaseSchema = z.enum(['starting', 'running', 'stopping', 'ended', 'failed'])
...
const terminal = (phase: ExecutionPhase) => phase === 'ended' || phase === 'failed'
...
export class SessionExecutionManager {
  ...
  static async open(file: string, project: (record: ExecutionRecord) => Promise<void>) {
    ...
    for (const record of data.records) {
      ...
      if (!terminal(record.phase)) {
        ...
        record.events.push({ phase: 'ended', at: record.finishedAt, reason: record.reason })
  ...
  async recoverOrphans(roster: readonly SessionRecord[]): Promise<void> {
    for (const row of roster) {
      ...
      const record: ExecutionRecord = {
        ...
        phase: 'ended', requestedAt: Date.now(), finishedAt: Date.now(), reason: 'orphaned-owner',
        events: [{ phase: 'ended', at: Date.now(), reason: 'orphaned-owner' }],
      };
  ...
  async start<T>(request: ExecutionRequest, driver: ExecutionDriver<T>): Promise<T> {
    ...
    this.records.set(record.executionId, record)
    this.active.set(record.resumeId, { record, stop: driver.stop })
    try {
      await this.transition(record, 'starting')
      const launched = await driver.start(async activity => {
      ...
      const completion = launched.completed.then(
        result => this.finish(record, result.failed ? 'failed' : 'ended', result.reason),
        () => this.finish(record, 'failed', 'execution-failed'),
      );
  ...
  async run<T>(request: ExecutionRequest, spawn: (ready: (pid?: number) => void, signal: AbortSignal) => Promise<T>, classify: (value: T) => { reason: string; failed?: boolean }): Promise<T> {
    ...
    const running = await this.start(request, {
      start: async () => {
        result = spawn(pid => ready({ pid }), controller.signal)
      ...
      stop: async () => { controller.abort(); await result?.catch(() => undefined) },
    })
  ...
  async stop(resumeId: string, reason: string): Promise<boolean> {
    if (!reason.trim()) throw new Error('Stop reason is required')
    while (this.locks.has(resumeId)) await this.locks.get(resumeId)
    const entry = this.active.get(resumeId)
    if (!entry || terminal(entry.record.phase)) return false
    const release = this.lock(resumeId)
    try {
      let journalError: unknown
      try { await this.transition(entry.record, 'stopping', reason) } catch (error) { journalError = error }
      // A failed disk must prevent new launches, never prevent killing an existing child.
      await entry.stop(reason)
      try { await this.finish(entry.record, 'ended', reason, true) } catch (error) { journalError ??= error }
      if (journalError) {
  ...
  private async finish(record: ExecutionRecord, phase: 'ended' | 'failed', reason: string, explicit = false) {
    if (this.active.get(record.resumeId)?.record !== record || terminal(record.phase)) return
